...
- Record fresh cpace-x25519 OSS-Fuzz local build validation for all 15 registered native Go fuzz targets at `a2f892f785991b8ac20d60979c1f32639287f0d4`, with raw evidence under `docs/evidence/ossfuzz-a2f892f-20260704/`, and open the fresh upstream `google/oss-fuzz#15838` submission. Upstream review/merge and later ClusterFuzz signal remain pending before OSS-Fuzz onboarding is complete.
- Add a SageMath-derived extended X25519 vector dataset with a pinned Docker/Sage generation script, hash-pinned JSON fixture, generator derivation checks, random/low-order/twist/non-canonical scalar-multiplication cases, and full package-profile exchange ISK/tag/message vectors. Test-only: no public API, wire, protocol, or dependency change; exact-candidate evidence still needs refresh before release-current claims.
