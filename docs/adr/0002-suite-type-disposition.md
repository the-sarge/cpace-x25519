...
- **API diff is exact:** `apidiff v0.1.2 HEAD` reports exactly two removals (`type Suite`, `const SuiteCPaceRistretto255SHA512`) and no other API change. No exported identifier whose name contains `Suite` and no exported identifier of the now-internal suite type remains in `api.go`, `doc.go`, or any other file under the repo root.
