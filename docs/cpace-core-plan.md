...
**In front (`input.go` acceptance and `api.go` shell):** `Input` acceptance and normalization — *including* `buildCI`, which runs inside `normalizeInput` so `normalizedInput.ci` reaches the core prebuilt — live in `input.go`. Public shell methods and the unexported `startWithRandom` / `respondWithRandom` randomness wrappers stay in `api.go`, where they own public error wrapping, wire framing (`encode`/`decodeMessage*`), the single-use guard, the `a.sid == nc.sid` message-vs-config check, and the password backstop `defer`.
